...
**0. 挑戦を否定する言葉を書かない。この項目がすべてに優先します。**
   全文は `KOTOBA.md`。何かを書き始める前に必ず読んでください。
   書かない言葉：どうせ続かない／まだ生きてる／生き残った／生存率／失敗／
   終わった／消えた／淘汰／敗者／撤退／頓挫／寿命／打ち切り。
   代わりに：続いています／役目を終えました／次へ渡しました／挑戦。
